...
public class AchievimentsData : ScriptableObject
{
    ...
    private void OnValidate()
    {
        ...
        for (int i = 0; i < allAchiviements.Length; i++)
        {
            ...
            for (int j = i + 1; j < allAchiviements.Length; j++)
            {
                ...
                // Если behaviour ссылаются на один экземпляр — клонируем правый behaviour
    ...
    // Копирование полей объекта behaviour в новый экземпляр его типа
...
public class Achiviement
{
    ...
    // Публичное свойство для доступа/замены behaviour из других классов
    ...
    // Глубокий клон Achiviement (клонирует Info и создаёт новый экземпляр behaviour того же типа)
