...
public class BonusExecuter : MonoBehaviour
{
    public void Init()
    {
        ...
        GameG.GrappableObjectMediator.Subscribe<AddBulletsGrapAction>((action, grapObject) =>
        {
            GameG.Player.Gun.CountBullets.AddValue(action.AddBullets);
        });
        ...
        {
            GameG.ScoreSys.AddScore(action.AddScore, grapObject.transform.position);
        });
